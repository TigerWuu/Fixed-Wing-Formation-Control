...
bagPath = 'T:/NTU/Graduate/OneDrive - 國立台灣大學/NCSLab/FW-Formation-Control/rosbag/0906_rosbagreadtest';
topicName = '/wind_estimation_information';

% Create a rosbag reader object
reader = ros2bagreader(bagPath);

% Read messages from the specified topic
bagSel = select(reader,"Topic",topicName);
msgs = readMessages(bagSel);
%% 
% Extract timestamps and data
timestamps = cellfun(@(m) double(m.Header.Stamp.Sec) + double(m.Header.Stamp.Nanosec)*1e-9, msgs);
data = cellfun(@(m) m.Data, msgs); % Adjust this line based on your message structure
